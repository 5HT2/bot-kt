...
object IssueCommand : Command("issue") {
    init {
        ...
        string("mode") {
            string("user") {
                string("repoName") {
                    greedyString("issueNum") {
                        doesLater { context ->
                            val mode: String = context arg "mode"
                            ...
                            if (mode == "number") {
                                    ...
                                    if (result.html_url.contains("issue")) {
                                        message.channel.send {
                                            embed {
                                                ...
                                                field(
                                                    "Milestone",
                                                    result.milestone?.title ?: "No Milestone Added",
                                                    false
                                                )
                                                field(
                                                    "Labels",
                                                ...
                                                field(
                                                    "Assignees",
                                                    if ((result.assignees?.joinToString { it.login } ?: "No Assignees").isEmpty()) {
                                                        "No Assignees"
                                                    } else {
                                ...
                                try {
                                    val request = Request.Builder().addHeader("Authorization", "token $githubToken").url("https://api.github.com/search/issues?q=$user/$repoName+${issueNum.replace(" ", "%20")}&sort=created&order=asc").get().build()
                                    val response = OkHttpClient().newCall(request).execute()
                                    println(response.request().toString() + " from user ${message.author?.name}(${message.author?.id})")
                                    val result = Gson().fromJson(response.body()!!.string(), Search::class.java)
                                    message.channel.send(result.items?.get(0)?.html_url ?: "No results found.")
                                } catch(e: Exception){
                                    message.channel.send {
                                        embed {
                                            title = "Error"
    ...
    override fun getHelpUsage(): String {
        return "Getting information of an issue/pull on github. \n\n" +
                "Usage: \n" +
                "`;$name <user/organization> <repository> <issue | query>`\n\n" +
                "Example for getting issue by number: \n" +
                "`;$name kami-blue bot-kt 10`\n" +
                "Example for getting issue by search: \n" +
                "`;$name kami-blue client baritone pathfinding`"
    }
}
