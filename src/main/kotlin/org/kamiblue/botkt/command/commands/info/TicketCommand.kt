...
object TicketCommand : BotCommand(
    ...
    suspend fun onMessageReceive(event: MessageReceiveEvent) {
        ...
        if (!Main.ready
            || author?.bot == true
            || author?.id?.hasPermission(PermissionTypes.COUNCIL_MEMBER) == true
        ) return

        config?.ticketCreateChannel?.let {
            if (event.message.channel.id != it) return

            val everyone = event.message.server?.id?.let { serverId -> event.message.server?.roles?.find(serverId) } ?: return

            config.ticketCategory?.let { configTicketCategory ->
                val ticketCategory = server?.channelCategories?.find(configTicketCategory) ?: return
                ...
                ticket.edit {
                    ...
                    rolePermissionOverwrites.add(RolePermissionOverwrite(everyone, PermissionSet(0), PermissionSet(3072)))
                }
